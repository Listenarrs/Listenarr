...
namespace Listenarr.Tests.Features.Application.Downloads
{
    ...
    public class DownloadProcessingJobProcessorTests : BaseTests
    {
        ...
        public override async Task InitializeAsync()
        {
            ...
            /*downloadImportServiceMock.Setup(m => m.ImportDownloadFilesAsync(It.IsAny<Audiobook>(), It.IsAny<List<string>>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync((Audiobook audiobook, List<string> files, CancellationToken _) =>
                {
                    if (files.Count <= 0)
                    {
            ...
            _services.AddSingleton(downloadImportServiceMock);*/
            Init();
        }
