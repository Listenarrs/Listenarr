...
### Added
- **Per-root-folder storage:** The System page storage card now lists available space for each configured root folder alongside the application-data disk (config volume) and the system disk (container root, e.g. docker.img on Unraid), instead of only the disk hosting the application. Root folders on missing or inaccessible paths are shown as unavailable rather than failing the page. (#508)
