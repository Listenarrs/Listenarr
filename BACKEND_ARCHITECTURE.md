...
| `DirectDownloadService` | `DirectDownloadProcessor` | Owns internal DDL transfer state: `Queued -> Downloading -> Completed/Failed`, writes the trusted remote file to local staging, updates progress, and enqueues the import job after the local file is durable. | Must not import files, mark imports final, poll external download clients, or clean up moved downloads. | Periodic polling; failed HTTP/file writes transition the DDL record to `Failed` so active deduplication is released and the UI stops showing a stuck queued item. | Only rows with `DownloadClientId == "DDL"` and a supported direct-download source policy are fetched; the selected policy validates the original URL and every redirect target, and partial files are written with a `.partial` suffix and replaced atomically on success. | `DownloadProcessingJobService` receives completed local DDL files for import. |
...
1. `DownloadMonitorService` observes the external client and persists `Completed`; for DDLs, `DirectDownloadService` downloads the trusted file locally and persists `Completed`.
2. DDL sources are additive through infrastructure source policies. Adding a new DDL source should not require changing `DirectDownloadProcessor`; each source must provide explicit allow-list validation for initial URLs and redirects.
