        ...
        public async Task<List<ImportResult>> ImportDownloadFilesAsync(Audiobook audiobook, List<string> files, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    batchDestinationRoot = audiobook.BasePath;
                    ...
                    foreach (var file in orderedFiles)
                    {
                        ...
                        if (!FileUtils.IsAudioFile(file))
                        {
                            ...
                            try
                            {
                                ...
                                res.Success = await fileMover.PerformActionOn(completedFileAction, file, finalCompanionPath);
                                res.FinalPath = finalCompanionPath;
                                res.Action = completedFileAction;
                            }
                            catch (Exception ex) when (ex is not OperationCanceledException && ex is not OutOfMemoryException && ex is not StackOverflowException)
                            {
                                res.Success = false;
                                res.Message = ex.Message;
                                logger.LogWarning(ex, "ImportFilesFromDirectory: Failed companion-file import for {File}", file);
                            }
                        ...
                        try
                        {
                            ...
                            try
                            {
                                if (audiobook.Files != null && audiobook.Files.Count != 0 && !IsQualityBetter(candidateQuality, bestExisting, abProfile))
                                {
                                    ...
                                    results.Add(res);
                                    logger.LogInformation("ImportFilesFromDirectory: Skipping import of file {File} for audiobook {AudiobookId} because candidate quality '{Candidate}' is not better than existing '{Existing}'", file, audiobook.Id, candidateQuality, bestExisting);
                                    continue; // skip importing this file
                                }
                            }
                            catch (Exception ex) when (ex is not OperationCanceledException && ex is not OutOfMemoryException && ex is not StackOverflowException)
                            {
                                logger.LogDebug(ex, "ImportFilesFromDirectory: Failed to evaluate quality for multi-file import {File}", file);
                            }
                            ...
                            res.Success = await fileMover.PerformActionOn(completedFileAction, file, destPathForFile);
                            res.FinalPath = destPathForFile;
                            res.Success = true;
                            ...
                            try
                            {
                                ...
                                var created = await audiobookFileService.EnsureAudiobookFileAsync(audiobook, res.FinalPath, "download");
